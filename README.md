...
Также выполнено **дополнительное задание**: для конструкции `do-while` построены AST и CST из лабораторной работы №5, реализована генерация трёхадресного кода и две локальные оптимизации промежуточного представления.
...
```mermaid
flowchart TD
    A[Начало] --> B[Взять очередную TAC-инструкцию]
    B --> C{Операция арифметическая?}
    C -- Да --> D{Оба аргумента - числа?}
    D -- Да --> E[Вычислить результат]
    E --> F[Заменить операцию на присваивание]
    D -- Нет --> G[Оставить инструкцию без изменений]
    C -- Нет --> H{Это jfalse с константой?}
    H -- Да --> I{Константа равна 0?}
    I -- Да --> J[Заменить на безусловный переход]
    I -- Нет --> K[Заменить на nop или удалить]
    H -- Нет --> G
    F --> L{Есть ещё инструкции?}
    G --> L
    J --> L
    K --> L
    L -- Да --> B
...
```mermaid
flowchart TD
    A[Начало] --> B[Создать таблицу соответствий переменная -> значение]
    B --> C[Взять очередную инструкцию]
    C --> D{Arg1 или Arg2 есть в таблице?}
    D -- Да --> E[Подставить известное значение]
    D -- Нет --> F[Оставить аргументы без изменений]
    E --> G{Инструкция является присваиванием?}
    F --> G
    G -- Да --> H[Запомнить Result -> Arg1]
    G -- Нет --> I{Инструкция изменяет переменную?}
    I -- Да --> J[Удалить устаревшее соответствие]
    I -- Нет --> K[Оставить таблицу без изменений]
    H --> L{Есть ещё инструкции?}
    J --> L
    K --> L
    L -- Да --> C
    L -- Нет --> M[Конец]
