...
1. Открыть решение проекта в **Visual Studio 2022**.
2. Убедиться, что текстовые файлы из папки `Texts` добавлены в проект.
3. Для файлов `task.txt`, `grammar.txt`, `class.txt`, `method.txt`, `test.txt`, `refs.txt`, `source.txt` установить:
   - **Build Action:** `Content`
   - **Copy to Output Directory:** `Copy if newer`
4. Запустить проект клавишей **F5** или кнопкой **Start**.

### Запуск готовой сборки

При наличии опубликованной версии программы:

1. Скачать архив с готовой сборкой.
2. Распаковать его в отдельную папку.
3. Запустить файл `GUI.exe`.
...
Для распознавания лексем был спроектирован конечный автомат с начальным состоянием `START`.

Из начального состояния выполняются переходы:

- в состояние распознавания идентификатора или ключевого слова;
- в состояние распознавания числа;
- в состояние обработки пробела или табуляции;
- в состояние распознавания разделителей;
- в состояние распознавания операторов;
- в состояние ошибки.
