...
Согласно классификации Хомского, грамматика `G[<DW>]` относится к классу контекстно-свободных грамматик. По определению, контекстно-свободная грамматика имеет правила вида `A → α`, где `A ∈ VN`, `α ∈ V*`, при этом общий словарь грамматики определяется как `V = VT ∪ VN`.
...
```md
![Схема рекурсивного спуска](images/00_recursive_descent_scheme.png)
```

Файл схемы рекомендуется сохранить под именем `00_recursive_descent_scheme.png`.
...
```md
![Корректный пример анализа конструкции do-while](images/01_correct_do_while.png)
```
...
```md
![Ошибка: пропущен символ ; после оператора ++](images/02_missing_semicolon_after_increment.png)
```
...
```md
![Пример комплексной диагностики нескольких ошибок](images/03_multiple_errors.png)
```

### 9.4. Пропущено выражение после оператора присваивания
...
### 9.6. Строка без первого ключевого слова `do`
...
### 9.7. Отсутствует точка с запятой в конце конструкции
...
Ожидаемый результат: сообщение о том, что отсутствует символ `;` в конце конструкции `do-while`.
